...
async GetChaptersByHot(type?: string, page?: number, order?: string) {
    try {
        ...
        const resultList: IResultSearch<IMangaList> = {
            results: data.map((e: { id: number; chap: string; hid: string; lang: string; publish_at: string; created_at: string; last_at: string md_comics: { title: string; slug: string; md_covers: { b2key: string }[] } }) => {
